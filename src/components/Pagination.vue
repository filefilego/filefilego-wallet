    ...
    <div>
        <ul v-show="total > 0" class="pagination">
            <li :class="canGoToFirstLast('first','page-item')">
                <router-link aria-label="First" class="page-link" :to="'?page='+1"><span aria-hidden="true">««</span></router-link>
            </li>
            <li :class="canGoToNextPrevious('previous', page-1, 'page-item')">
                <router-link aria-label="Previous" class="page-link" :to="'?page='+(page-1)"><span aria-hidden="true">«</span></router-link>
            </li>
            <li v-show="size!='s'" v-for="r in range" :key="r.page" :class="getSelectedClass(r, 'page-item')">
                <router-link v-show="r.page!='...'" class="page-link" :to="'?page='+ r.page"> {{r.page}} <span v-show="isCurrentPage(r)" class="sr-only"></span></router-link>
                <router-link v-show="r.page=='...'" class="page-link" to="#"> {{r.page}} </router-link>
            </li>
            <li class="page-item active" v-show="size=='s' && range.length > 0">
                <router-link class="page-link" :to="'?page=' + getCurrentPage().page"> {{getCurrentPage().page}} <span class="sr-only"></span></router-link>
            </li>
            <li :class="canGoToNextPrevious('next', page+1, 'page-item')">
                <router-link aria-label="Next" class="page-link" :to="'?page='+(page+1)"><span aria-hidden="true">»</span></router-link>
            </li>
            <li :class="canGoToFirstLast('last','page-item')">
                <router-link aria-label="Last" class="page-link" :to="'?page=' + last_page"><span aria-hidden="true">»»</span></router-link>
            </li>
        </ul>
    </div>
</template>
