  ...
  <div v-show="!entryMode">
    <div style="border-bottom: 1px solid #ddd" uk-grid>
      ...
      <div class="uk-width-auto">
        <div>
          <div >
            <span class="clickable icon-plus" style="color: #3e15ca; z-index: 99999999; display:inline-block;  cursor: pointer; border: 2px solid; border-radius: 50%; padding: 5px; vertical-align: middle; font-size: 1.1em;" ></span>
            <div id="new_dropdown" style="background-color:white; border:1px solid #e3e3e3; " uk-dropdown="mode: click; delay-hide:0;">
              <ul style="padding:0px; margin:0px;" class="chan-action-ul uk-list">
                  <li @click="openSubchannelModal" class="clickable"> <span class="icon-tree"></span> New sub-channel </li>
                  <li @click="openEntryModal" class="clickable"> <span class="icon-stack"></span> New entry </li>
                  <li class="clickable"> <span class="icon-folder"></span> New folder </li>
                  <li class="clickable"> <span class="icon-upload"></span> New file </li>
              </ul>
            </div>
          </div>
        </div>
          ...
          <div v-show="!loadingBalance" class="uk-text-center" style="padding: 15px">
           ...
           <div style="margin-top: 15px">
              ...
              <div class="uk-margin uk-text-center" style="padding: 15px">
                ...
                <span v-show="channel_creation_error != ''" style="color:red; margin-top:8px; display:inline-block;">{{channel_creation_error}}</span>
                <br />
...
export default {
  data() {
    return {
      ...
      colors: ["#3e15ca", "#f0506e", "#32d296", "#ff6200", "#00cca6", "#4947ef", "#bdbdbd"],
  ...
  methods: {
    ...
    async createSubChannel() {
      ...
      try {
        ...
        if (!this.selected_wallet_status.unlocked) {
          this.channel_creation_error = "You have to select a wallet and unlock it first. Go to 'Wallet' section."
          return;
        }
        ...
        let txFee = new BN(this.blockchain_settings.node_creation_fees_guest, 10);

        // console.log(balance, balanceFinal);
        if(balance.lt(txFee)) {
          this.channel_creation_error = "* Sub channel creation requires " + unitUtil.fromAran(txFee.toString(10), "zaran").toString(10) +" ZARANS. You don't have enough balance"
          return
        }
        ...
        if(!foundTx) {
          this.channel_creation_error = "It may take a few seconds for your sub channel to be available. Please search for it in the channel list!"
        }

      } catch (e) {
        this.channel_creation_error = "Unexpected error. Make sure you are connected to internet!"
      } finally {
    ...
    async getNode(hash) {
      ...
      if (res.data.result == null || (res.data.result && !res.data.result.childs) ) {
        this.subchans = [];
        this.data = [];
      } else if (res.data.result && res.data.result.childs) {
        this.subchans = res.data.result.childs.filter((o) => o.NodeType == 3);
        this.data = res.data.result.childs.filter((o) => o.NodeType != 3);
      }
    },
  },
  watch: {
